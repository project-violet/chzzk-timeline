...
async fn run_live_chat_test() -> Result<()> {
    utils::log("실시간 채팅 수집 모드 시작");
    api::websocket::spawn_live_chat_ws_server("0.0.0.0:12003");
    api::scan_channels().await?;

    // 최소한의 스레드를 계속 살아 있게 유지
    loop {
...
async fn run_analysis_chat(opts: &AnalysisChatOpt) -> Result<()> {
    ...
    // 고유 이용자가 많은 데이터로 제한 (샘플)
    utils::log("고유 이용자 필터 적용 중...");
...
fn load_channels_and_chat_logs(
    ...
    utils::log(format!("채팅 분석 모드 시작: {}개의 파일", file_paths.len()));
    ...
    utils::log(format!("채팅 로그 폴더에서 최종 로드: {}", chat_logs_dir));
...
fn run_channel_distance_analysis(
    ...
    // 관련 채널 링크를 별도 JSON으로 저장 (각 채널당 최대 6개, distance >= 0.1)
...
fn run_cluster_similar_replays(channels: &Vec<ChannelWithReplays>, chat_logs: &Vec<ChatLog>) {
    utils::log("유사한 다시보기 클러스터링(매우 느리므로 기본 off)...");
